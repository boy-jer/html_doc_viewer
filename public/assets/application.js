...
;
// an abstraction to handle page fetching/caching transparently to the caller
var pageFetcher = function(pg_count, fetch_url) {
	this.page_count = pg_count;
	this.url = fetch_url;
	
	this.getPage = function(page_num) {
		if (false) { // look in cache
			//return pagesCache[page_num];
		}
		else { // fetch the page again
			var page_url = this.url + '-' + page_num + '.html';
			$.ajax({
				url: page_url,
