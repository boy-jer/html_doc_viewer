...
describe DocumentsController do
  ...
  describe 'display' do
    it 'return the document display page with success' do
      ...
      assigns(:url).should == "#{fetch_html_documents_url}?fetch_url=#{CONVERSION_SERVER}/1ef18bea-2d6f-408e-a742-3ddbd8c1d69e/test"
    end
  ...
  it 'returns the upload result page with success' do
    get :result
    response.should be_success
  ...
  it 'fetches the html content using the URL with success' do
    get :fetch_html, {:fetch_url => 'http://test.host/422.html'}
    response.should be_success
  end
